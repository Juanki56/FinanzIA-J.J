import type { CorreoParseado, Grupos5, Grupos6, PlantillaBancaria, RegistroBanco } from './types.js';
...
const PLANTILLAS: PlantillaBancaria[] = [
  ...
  {
    // Plantilla 11: Transferencia por llave Bre-B (con nombre del destinatario)
    // "Bancolombia: JUAN, transferiste $160,000.00 a la llave 8127086 desde tu cuenta *9247 a ALVARO ANDRES ARISTIZABAL SERNA el 13/09/26 a las 15:11."
    // Distinta de transferencia_enviada (esa va "a la cuenta *X") y de pago_qr
    // (ese dice "por codigo QR"). El nombre del usuario antes de "transferiste"
    // (aquí solo el primer nombre, no el completo como en pago_qr) tampoco se
    // captura — el regex empieza a buscar desde "transferiste" en adelante.
    nombre: 'transferencia_llave_breb',
    regex: /transferiste \$([\d.,]+) a la llave (\S+) desde tu cuenta \*(\d+) a (.+?) el (\d{2}\/\d{2}\/(?:\d{2}|\d{4})) a las (\d{2}:\d{2})/,
    interpretar(grupos: string[]): CorreoParseado {
...
export function parsearCorreoBancolombia(texto: string): CorreoParseado | null {
  for (const plantilla of PLANTILLAS) {
    ...
    if (match) {
      return plantilla.interpretar(match.slice(1));
    }
  }
  return null;
}
